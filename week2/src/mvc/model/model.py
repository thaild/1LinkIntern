...
class Model(object):
	...
	def write_add(self, file_name, _modes, _tmp):
		fo = open(file_name, _modes)
		for std in _tmp:
			_std = str(std).lstrip('[').rstrip(']').replace(",", "").replace("'", "", 2).strip()
			_id, name, add, sex, math, phy, chem = _std.split()
			if float(math) < 11 or float(phy) < 11 or float(chem) < 11:
				_str = std + '\n'
				fo.writelines(_str)
			else:
				print ("Error score!")
