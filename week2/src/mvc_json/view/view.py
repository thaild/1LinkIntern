...
class View(object):
	...
	def _edit_option(self, _list):
		...
		for i in range(_list.__len__()):
			if int(_list[i]["id"]) == id_edit:
				print (str(json.dumps(_list[i])))
				...
				while True:
					...
					if choice == 0:
						...
						print("Not Exist!")
					print (str(json.dumps(_list[i])))
					Model().write('input.json', 'w', self._tmp)
					return _list
	...
	def _search_option(self, _list):
		...
		while True:
			...
			if choice == 0:
				...
				self._tmp = Controller().con_search(str(choice), _list, find_str)
				print (self._tmp)
			else:
				print("Not EXISTS")
	...
	def view_student(self, students):
		for student in students:
			print (student["id"] + ' | ' + student["name"] + ' | ' + student["add"] + ' | ' + student["gender"] + ' | ' + str(student["scores"]["math"]) + ' | ' + str(student["scores"]["physics"]) + ' | ' + str(student["scores"]["chemistry"]))

	def main_view(self):
		menu = "..::STUDENT MANAGER::..\n1. Enter infor student form keyboard\n2. View list Student\n3. Edit a Student\n4. Delete a Student\n5. Search\n6. Statistics\n0. Exit"
		list_std = Model().read('input.json')
		...
		while True:
			...
			if choice == 0:
				...
				self._tmp = self._add_student()
				# self.view_student(self._tmp)
				Controller().con_add(self._tmp)
			elif choice == 2:
				print ('List Student:')
				students = (Controller().con_main())
				self.view_student(students)
			elif choice == 3:
