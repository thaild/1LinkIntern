...
# read tu file, neu gap line trong or k du student thi bo qua??
#
	...
	def _search_score(self, _score):
		...
		find = float(_score)
		for i in range(self._list.__len__()):
			if float(self._list[i].get_math()) == find or float(self._list[i].get_phys()) == find or float(self._list[i].get_chem()) == find:
				_total += 1
				print (self._list[i])
				write_str.append(str(self._list[i]))
		_str = str(write_str).lstrip('[').rstrip(']').replace(",", "\n", _total).replace("'", "", _total * 2).strip()
